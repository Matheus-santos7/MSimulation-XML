...
├── routes/auth/
│   ├── index.ts                     # handlers HTTP (auth + sessão)
...
│   ├── onboarding/
│   │   └── onboarding-service.ts    # attachTenant (primeira empresa)
│   ├── email/                       # verificação, reset, Resend
│   └── mfa/two-factor-service.ts    # TOTP
├── lib/org/
│   ├── db-errors.ts                 # isPrismaUniqueError (compartilhado)
│   └── tenant-mapper.ts
├── lib/
...
└── lib/auth/
    ...
    ├── api/
    │   ├── client.ts                # postAuthJson, authBearerFetch, AuthApiError
...
└── lib/onboarding/
    ├── api.ts                       # onboardingTenantApi (authBearerFetch)
    ├── actions/
...
| `attachTenant()` | `POST /onboarding/tenant` | Primeira empresa → user vira ADMIN (`OnboardingService`) |
...
Onboarding usa `lib/onboarding/` (`api.ts` + `actions/`), separado de `lib/auth`.
...
*Atualizado em junho/2026 — `OnboardingService`, `lib/onboarding/actions/*`, `lib/auth/types/*` + `session.ts`, `lib/brand.ts`, `lib/auth/api/*`, `routes/auth/onboarding.routes.ts`, `lib/onboarding/api.ts`.*
