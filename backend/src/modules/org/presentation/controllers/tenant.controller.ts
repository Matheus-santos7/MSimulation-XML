...
export const tenantController: FastifyPluginAsync = async (app) => {
  ...
  /** Returns only the JWT tenant with filiais (compatible with UI expecting an array). */
  app.get("/tenants", async (request) => {
    const tenantId = tenantIdFromRequest(request);
    const tenant = await org.tenantRepository.findByIdWithFiliais(tenantId);
  ...
  app.get("/tenants/:id", async (request, reply) => {
    ...
    const tenant = await org.tenantRepository.findByIdWithFiliais(id);
  ...
  app.patch("/tenants/:id", async (request, reply) => {
    try {
      ...
      if (!tenant) return reply.status(404).send({ error: "Empresa não encontrada" });
      return org.tenantRepository.findByIdWithFiliais(id);
    } catch (error) {
