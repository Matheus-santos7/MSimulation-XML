...
function hasXlsxMagicBytes(data: Uint8Array): boolean {
...
export function validateSpreadsheetBuffer(
  ...
  if (fileName && !fileName.endsWith(".xlsx") && !fileName.endsWith(".xls")) {
    return { ok: false, error: "Formato inválido. Envie um arquivo .xlsx" };
  }
...
export async function validateSpreadsheetFile(
  file: File,
  options: Omit<SpreadsheetValidationOptions, "fileName" | "mimeType"> = {},
