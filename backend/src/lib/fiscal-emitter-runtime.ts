...
export function resolveModFrete(settings: FiscalEmitterSettingsData, tipo: NFeTipo): string {
...
export function composicaoChannel(tipo: NFeTipo): keyof ComposicaoLinha {
...
function applyDifal(
  ...
  if (!aplica || mode === "SEM_DIFAL") {
    return { valorIcms: valor * (asNum(snapshot.icms.aliquota, 0) / 100), vDifal: 0, aplica: false };
  }
...
export async function loadEmitterSettings(
  prisma: { fiscalEmitterSettings: { findUnique: (args: { where: { tenantId: string } }) => Promise<{ settings: unknown } | null> } },
  tenantId: string,
