...
import { resolveProductForAvanco } from "./avanco-product-resolve.js";
...
function createPrismaMock() {
  ...
  } as unknown as Parameters<typeof resolveProductForAvanco>[0];
}

describe("resolveProductForAvanco", () => {
  it("resolve pelo SKU do tenant mesmo quando o saldo FIFO usa product_id legado", async () => {
    const result = await resolveProductForAvanco(
      createPrismaMock(),
      tenantId,
      productIdLegado,
      sku,
    ...
    assert.equal(result.product.id, productIdCadastro);
